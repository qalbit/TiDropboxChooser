...
#define TAG "ti.dropboxchooser"

using namespace v8;

static Persistent<Object> bindingCache;

static void TiDropboxChooser_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiDropboxChooser.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TiDropboxChooserBindings::lookupGeneratedInit(
...
static void TiDropboxChooser_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiDropboxChooser_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiDropboxChooser_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TiDropboxChooserBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiDropboxChooserBinding = {
	"ti.dropboxchooser",
	TiDropboxChooser_init,
	TiDropboxChooser_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_dropboxchooser_TiDropboxChooserBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.dropboxchooser", &TiDropboxChooserBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TiDropboxChooserBindings::lookupGeneratedInit));
}
